  ...
  <div class="min-h-screen bg-[var(--color-bg-dark)] flex font-sans transition-colors duration-300">
    ...
    >
      ...
      <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        <NuxtLink v-if="['owner', 'admin'].includes(userRole)" @click="isMobileMenuOpen = false" to="/app/settings/audit" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200" active-class="bg-[var(--color-accent-blue)]/10 text-[var(--color-accent-blue)] shadow-[0_0_20px_rgba(0,113,227,0.15)]" :class="$route.path.includes('audit') ? '' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-white)] hover:bg-[var(--color-bg-subtle)]'">
