          ...
          <form @submit.prevent="handleSignup" class="space-y-6">
            ...
            <div>
              ...
              <div class="mt-2 relative">
                 ...
                 <button type="button" :aria-label="showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-3 flex items-center text-[var(--color-text-secondary)] hover:text-[var(--color-white)] cursor-pointer focus:outline-none">
            ...
            <div>
              <label for="confirmPassword" class="block text-sm font-bold text-[var(--color-text-secondary)]">Confirmar Contraseña</label>
              <div class="mt-2 relative">
                 <input id="confirmPassword" v-model="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'" required class="appearance-none block w-full px-4 py-3 border border-[var(--color-border-subtle)] rounded-xl shadow-sm placeholder-[var(--color-text-secondary)]/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-blue)] focus:border-transparent bg-[var(--color-bg-subtle)] text-[var(--color-white)] sm:text-sm transition-all pr-10" placeholder="Repite tu contraseña" />
                 <button type="button" :aria-label="showConfirmPassword ? 'Ocultar confirmación de contraseña' : 'Mostrar confirmación de contraseña'" @click="showConfirmPassword = !showConfirmPassword" class="absolute inset-y-0 right-0 pr-3 flex items-center text-[var(--color-text-secondary)] hover:text-[var(--color-white)] cursor-pointer focus:outline-none">
